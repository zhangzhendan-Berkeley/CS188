...
class PerceptronModel(Module):
    ...
    def forward(self, x):
        ...
        """
        x = x.view(-1, 1)
        score = tensordot(self.w, x, dims=1)  # shape (1,)
...
class LanguageIDModel(Module):
    ...
    def forward(self, xs):
        ...
        memory = zeros(batch_size, self.hidden_size)
        for x in xs:
            memory = relu(self.linear1(x) + self.linear2(memory))

        output = self.linear4(memory)
        return output
...
def Convolve(input: tensor, weight: tensor):
    ...
    H, W = input.shape
    kH, kW = weight.shape
    Output_Tensor = zeros((H - kH + 1, W - kW + 1))
    for x in range(0,W-kW+1):
        for y in range(0,H-kH+1):
            horizon = input[y:y+kH, x:x+kW].flatten()
            Output_Tensor[y][x] = tensordot(horizon,weight.flatten(), dims=1)
    "*** End Code ***"
...
class DigitConvolutionalModel(Module):
    ...
    def __init__(self):
        ...
        self.linear4 = Linear(64, output_size)
...
class Attention(Module):
    ...
    def forward(self, input):
        ...
        att = torch.matmul(Q, K.transpose(1, 2)) / (C ** 0.5)
        att = att.masked_fill(self.mask[:, :, :T, :T] == 0, float('-inf'))
        att = softmax(att, dim=-1)
        out = torch.matmul(att, V)  # (B, T, C)

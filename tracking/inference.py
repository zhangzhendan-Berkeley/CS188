...
def constructBayesNet(gameState: hunters.GameState):
    ...
    edges = [(PAC,OBS0), (PAC, OBS1), (GHOST1,OBS1), (GHOST0,OBS0)]
    variableDomainsDict = {}
    ...
    variableDomainsDict[OBS0] = list(range(maxDistance + MAX_NOISE + 1))
    variableDomainsDict[OBS1] = list(range(maxDistance + MAX_NOISE + 1))
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for var in eliminationOrder:
            # join 所有包含 var 的因子
            factorsWithoutVar, joinedFactor = joinFactorsByVariable(factors, var)

            # 如果该变量出现在任何因子中（joinedFactor 非 None）
            if joinedFactor is not None:
                # 如果因子剩下多个未条件变量，可以消元
                if len(joinedFactor.unconditionedVariables()) > 1:
                    eliminatedFactor = eliminate(joinedFactor, var)
                    factors = factorsWithoutVar + [eliminatedFactor]
                else:
                    # 否则直接丢弃该因子（无效）
                    factors = factorsWithoutVar
            else:
                # 没有包含 var 的因子，继续
                factors = factorsWithoutVar

        # === Step 3. 将剩余因子 join 起来并 normalize ===
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        if total == 0:
            return None  # 理论上不会出现，因为题目保证非空且非零
...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for oldPos in self.allPositions:
            ...
            if oldPos not in distCache:
                distCache[oldPos] = self.getPositionDistribution(gameState, oldPos)
            newPosDist = distCache[oldPos]  # DiscreteDistribution

            for newPos, transProb in newPosDist.items():
                newBeliefs[newPos] = newBeliefs.get(newPos, 0.0) + prior * transProb
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        # Step 1. 计算每个粒子的观测概率权重
        for p in self.particles:
            weights[p] = weights.get(p, 0) + self.getObservationProb(
                observation, pacmanPos, p, jailPos
            )
